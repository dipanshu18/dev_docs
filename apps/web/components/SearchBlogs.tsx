...
export default function SearchBlogs() {
  ...
  async function handleBlogSearch(e: FormEvent, searchTerm: string) {
    e.preventDefault();
    ...
    <div className="max-w-2xl mx-auto my-5 p-5">
      <label className="input input-bordered flex items-center gap-2">
        <input
          ...
          className="grow"
          onChange={(e) => {
            handleBlogSearch(e, e.target.value);
          }}
          placeholder="Search"
        />
        <svg
      ...
      {searchResults && (
        <div className="bg-gray-950 p-4 my-1 rounded-2xl">
      ...
      {searchError && (
        <div className="bg-gray-950 py-5 px-4 my-1 rounded-2xl">
          {searchError}
        </div>
      )}
    </div>
